...
class FlowPreProcessingParameters:
    def __init__(
        self,
        minimum_turbineflow,
        minimum_turbineflow_percent,  # Flow parameters
        ...
        self.minimum_turbineflow = (
            minimum_turbineflow  # minimum flow through the turbine (m3/s)
        )
        ...
        self.annual_maintenance_flag = (
            annual_maintenance_flag  # Boolean to include annual maintenance or not
        )
        self.major_maintenance_flag = (
            major_maintenance_flag  # Boolean to include major maintenance or not
        )
...
class FlowPreProcessing:
    ...
    def max_turbineflow_checker(self, flow_obj):
        ...
        flow_obj.turbine_flow = np.where(
            flow > design_flow, design_flow, flow
        )  # turbine_flow is created here as this code is always active and runs 1st. flow is < design flow
    ...
    def min_turbineflow_checker(self, flow_obj):
        ...
        if flow_obj.minimum_turbineflow is not None:
            ...
            if (
                flow_obj.minimum_turbineflow_percent is not None
            ):  # Check if a user entered min flow percentage if not, use 10% as default
                min_flow_percent = flow_obj.minimum_turbineflow_percent
            else:
                min_flow_percent = 10  # Default

            min_flow = (
                min_flow_percent / 100
            ) * design_flow  # Compute minimum flow that passes through the turbine
            flow_obj.minimum_turbineflow = min_flow  # update

        flow_obj.turbine_flow = np.where(
            flow < min_flow, 0, flow
        )  # replace flows less than min flow with 0

    def annual_maintenance_implementer(self, flow_obj):
        # Function to set annual maintennace - i.e., make the flow 0 for a week a year

        turbine_flow = (
            flow_obj.turbine_flow
        )  # turbine flow series with max and min (if on) limits implemented
        datetime_index = flow_obj.datetime_index  # datetime index of the user flow
        flow = pd.DataFrame(
            data=turbine_flow, index=datetime_index, columns=["flow_cms"]
        )  # create dataframe with date and flow in m3/s
        ...
        )  # Years where there will be maintenance - every year

        # Find minimum week - maintennance week
        weekly = (
            flow.groupby(pd.Grouper(level="dateTime", freq="W")).mean().iloc[1:-1]
        )  # compute the two-weekly mean and drop first and last
        ...
        for m in maint_years:
            start_days.append(
                datetime.strptime(str(m) + str(week_maint) + "1", "%Y%W%w")
            )  # Monday of the week where maintenance starts
        ...
        for d in start_days:
            # for day in range(7):
            dates_maint.append(
                pd.date_range(d, periods=7).date
            )  # generate 7 days for each year
        ...
        flow.loc[flow.date.isin(dates_maint), "flow_cms"] = (
            0  # replace flow in minimum weeks with 0
        )
    ...
    def major_maintenance_implementer(self, flow_obj):
        ...
        end_date = datetime_index[-1].year  # End year
        maint_years = list(
            range(start_date, end_date, 5)
        )  # Years where there will be maintenance - every 5 years
        del maint_years[0]  # remove the first year

        # Find minimum two-week period - maintennance week
        weekly = (
            flow.groupby(pd.Grouper(level="dateTime", freq="2W")).mean().iloc[1:-1]
        )  # compute the two-weekly mean and drop first and last
        weekly["week"] = weekly.index.strftime("%W")
        week_maint = weekly.groupby("week").mean().idxmin()[0]
        ...
        for d in start_days:
            dates_maint.append(
                pd.date_range(d, periods=14).date
            )  # generate 14 days for each year
