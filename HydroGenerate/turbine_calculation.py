...
max_flow_turbine = (
    1  # multiplier for the maximum flow that can be passed through a turbine
)
...
class TurbineParameters:
    ...
    def __init__(
        ...
        self.design_flow = (
            design_flow  # desing flow of the hydropower system, cfs or m3/s
        )
        ...
        self.generator_efficiency = (
            generator_efficiency  # efficiency of the generator, default: 98%
        )
        ...
        self.pctime_runfull = (
            pctime_runfull  # percent of time a turbine will run full, default: 80%
        )
        self.pelton_n_jets = (
            pelton_n_jets  # number of jets in a Pelton turbine, default: 3
        )
        self.flow_column = flow_column  # when flow is a pandas dataframe, name of the column containing the flow data

        self.hk_blade_diameter = (
            hk_blade_diameter  # Hydrokinetic turbine blade diameter m
        )
...
def turbine_type_selector(hp_params):
    ...
    if polygon_kaplan.contains(point):
        ...
        centroids_list.append(
            math.sqrt(
                (point.x - centroid_kaplan.x) ** 2 + (point.y - centroid_kaplan.y) ** 2
            )
        )
    ...
    if polygon_francis.contains(point):
        ...
        centroids_list.append(
            math.sqrt(
                (point.x - centroid_francis.x) ** 2
                + (point.y - centroid_francis.y) ** 2
            )
        )
    ...
    if polygon_pelton.contains(point):
        ...
        centroids_list.append(
            math.sqrt(
                (point.x - centroid_pelton.x) ** 2 + (point.y - centroid_pelton.y) ** 2
            )
        )
    ...
    if polygon_turgo.contains(point):
        ...
        centroids_list.append(
            math.sqrt(
                (point.x - centroid_turgo.x) ** 2 + (point.y - centroid_turgo.y) ** 2
            )
        )
    ...
    if polygon_crossflow.contains(point):
        ...
        centroids_list.append(
            math.sqrt(
                (point.x - centroid_crossflow.x) ** 2
                + (point.y - centroid_crossflow.y) ** 2
            )
        )
    ...
    if not turbines_list:  # the head and flow are in in range for any turbine
        raise ValueError(
            "Head or flow paramters are out of range for all turbine types supported in HydroGenerate"
        )
    else:
        turbines_dict = dict(
            zip(turbines_list, centroids_list)
        )  # create a dictionary with all turbine responsed
...
class ReactionTurbines:
    ...
    def runnersize_calculator(self, design_flow):
        if (
            design_flow > 23
        ):  # d > 1.8 - The formula in the document has an 'undefined' area
            k = 0.41
        else:
            k = 0.46
...
class FrancisTurbine(Turbine):
    ...
    def turbine_calculator(self, turbine):
        ...
        enq = (
            (nq - 56) / 256
        ) ** 2  # Specific speed adjustment to peak efficiency (^e_nq)
        ed = (0.081 + enq) * (
            1 - (0.789 * d ** (-0.2))
        )  # Runner size adjustment to peak efficiency (^e_d)
        ep = (
            (0.919 - enq + ed) - 0.0305 + 0.005 * turbine.Rm
        ...
        FlowRange().flowrange_calculator(
            turbine=turbine
        )  # generate a flow range from 60% to 120% of the flow given
...
class KaplanTurbine(Turbine):
    ...
    def turbine_calculator(self, turbine):
        ...
        ep = (
            (0.905 - enq + ed) - 0.0305 + 0.005 * turbine.Rm
        )  # Turbine peak efficiency (e_p)
        ...
        turbine.turbine_efficiency = (
            1 - 3.5 * ((Qp - Q) / Qp) ** 6
        ) * ep  # Efficiency at flows above and below Qp
        turbine.turbine_efficiency = np.where(
            turbine.turbine_efficiency <= 0, 0, turbine.turbine_efficiency
        )  # Correct negative efficiencies
        turbine.runner_diameter = d  # update
...
class PeltonTurbine(Turbine):
    ...
    def turbine_calculator(self, turbine):
        ...
        Q = turbine.turbine_flow
        turbine.turbine_efficiency = (
            1 - ((1.31 + 0.025 * j) * (abs(Qp - Q) / (Qp)) ** (5.6 + 0.4 * j))
        ) * ep  # Efficiency at flows above and below peak efficiency flow (e_q)
        turbine.runner_diameter = d  # update
...
class TurgoTurbine(Turbine):
    ...
    def turbine_calculator(self, turbine):
        ...
        turbine.turbine_efficiency = (
            turbine.turbine_efficiency - 0.03
        )  # Pelton efficiency - 0.03
        turbine.turbine_efficiency = np.where(
            turbine.turbine_efficiency <= 0, 0, turbine.turbine_efficiency
        )  # Correct negative efficiencies
...
class CrossFlowTurbine(Turbine):
    ...
    def turbine_calculator(self, turbine):
        ...
        Q = turbine.turbine_flow
        turbine.turbine_efficiency = (
            0.79 - 0.15 * ((Qd - Q) / Qd) - 1.37 * ((Qd - Q) / Q) ** 14
        )  # Efficiency (e_q)
        turbine.turbine_efficiency = np.where(
            turbine.turbine_efficiency <= 0, 0, turbine.turbine_efficiency
        )  # Correct negative efficiencies
...
class PercentExceedance(DesignFlow):
    ...
    def designflow_calculator(self, turbine):
        ...
        if (
            pe is not None
        ):  # user defined percent of time a turbine is running full. pe ∝ 1/(design flow)
        ...
        if isinstance(flow, Number):
            design_flow = (
                flow  # when a single value of flow is provided, this is the design flow
            )

        else:
            pc_e = np.linspace(100, 1, 100)  # sequence from 100 to 1.
            flow = flow[~np.isnan(flow)]  # remove nan values
            flow_percentiles = np.percentile(
                flow, q=np.linspace(1, 100, 100)
            )  # percentiles to compute, 1:100
            flowduration_curve = {
                "Flow": flow_percentiles,
                "Percent_Exceedance": pc_e,
            }  # Flow duration curve
            design_flow = float(
                flowduration_curve["Flow"][
                    flowduration_curve["Percent_Exceedance"] == pe
                ]
            )  # flow for the selected percent of excedante
...
if __name__ == "__main__":
    ...
    turbine_parameters = TurbineParameters(
        turbine_type=None,
        flow=90,
        head=200,
        design_flow=100,
        ...
        hk_blade_type=None,
        hk_swept_area=None,
    )  # Initialize a TurbinePower instance
