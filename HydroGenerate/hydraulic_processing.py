...
class HydraulicDesignParameters:
    ...
    def __init__(
        self,
        flow,
        design_flow,  # Flow parameters
        head,
        head_loss,
        max_headloss_allowed,
        penstock_headloss_method,
        penstock_headloss_calculation,  # head / head loss
        penstock_length,
        penstock_diameter,
        penstock_material,
        penstock_frictionfactor,  # penstock parameters
        ...
        self.max_headloss_allowed = (
            max_headloss_allowed  # maximum head loss allowed (%, 1-100)
        )
        ...
        self.channel_average_velocity = (
            channel_average_velocity  # average cross sectional water velocity (Q/A)
        )
    ...
    def reynoldsnumber_calculator(self):
        ...
        self.Re = (
            4 * self.design_flow / (np.pi * self.penstock_design_diameter * nu)
    ...
    def designdiameter_calculator(self):
        ...
        if self.penstock_diameter is not None:  # penstock diameter is known
            ...
            if (
                self.penstock_design_diameter is None
            ):  # if design diameter is also none (not chekcking diameter)
...
class RoughnessCoefficients:
    def __init__(
        self, material, darcyweisbach_epsilon, hazenwiliams_c, mannings_n
    ):  # Epsilon, C, n
    ...
    class Values:  # roughness values from Epanet documentation
        def __init__(self):
            ...
            self.add_roughnesscoefficient_values(
                "CastIron", 0.00025908, 135, 0.0135
            )  # Cast Iron. Epsilon, C, n
            self.add_roughnesscoefficient_values(
                "Concrete", 0.0016764, 130, 0.0145
            )  # Concrete. Epsilon, C, n
            self.add_roughnesscoefficient_values(
                "GalvanizedIron", 0.0001524, 120, 0.016
            )  # Galvanized Iron. Epsilon, C, n
            self.add_roughnesscoefficient_values(
                "Plastic", 0.000001524, 145, 0.013
            )  # Plastic. Epsilon, C, n
            self.add_roughnesscoefficient_values(
                "Steel", 0.00004572, 145, 0.016
            )  # Steel. Epsilon, C, n

        def add_roughnesscoefficient_values(
            self, material, darcyweisbach_epsilon, hazenwiliams_c, mannings_n
        ):  # method for adding roughness values
            self.roughnesscoefficients[material] = RoughnessCoefficients(
                material, darcyweisbach_epsilon, hazenwiliams_c, mannings_n
            )
...
class DW_RoughnessSelector(Roughness):  # Darcy-Weisbach roughness selector
    ...
    def roughness_selector(self, material):
        roughness_coefficients = (
            RoughnessCoefficients.Values()
        )  # read existing roughness coefficient values
        if material is not None:  # user provided material
            epsilon = roughness_coefficients.roughnesscoefficients[
                material
            ].darcyweisbach_epsilon
...
class HW_RoughnessSelector(Roughness):  # Hazen-Williams roughness selector
    ...
    def roughness_selector(self, material):
        ...
        if material is not None:  # user provided material
            ...
            C = roughness_coefficients.roughnesscoefficients[
                "Steel"
            ].hazenwiliams_c  # defaul to steel
        return C
...
class Manning_RoughnessSelector(Roughness):
    ...
    def roughness_selector(self, material):
        ...
        if material is not None:  # user provided material
            ...
            n = roughness_coefficients.roughnesscoefficients[
                "Concrete"
            ].mannings_n  # defaul to concrete
        return n
...
class DW_FrictionFactor:  # Darcy-Weisbach friction factor calculator
    ...
    def frictionfactor_calculator(self, hydraulic_parameters):
        ...
        epsilon = DW_RoughnessSelector().roughness_selector(
            material=hydraulic_parameters.penstock_material
        )  # DW roughness factor
        ...
        if Re > 4000:
            ...
            )  # Swamee-Jain approximation to the Colebrook-White equation (Bhave, 1991)

        elif (
            Re > 2000
        ):  # Cubic interpolation formula derived from the Moody Diagram (Dunlop, 1991)
...
class DarcyWeisbach(HeadLoss):  # Head loss calculator using Darcy-Weisbach
    ...
    def penstock_headloss_calculator(self, hydraulic_parameters):
        ...
        if (
            hydraulic_parameters.penstock_frictionfactor is None
        ):  # if a value of ff is not given
            ff = DW_FrictionFactor().frictionfactor_calculator(
        ...
        if hydraulic_parameters.penstock_diameter is None:
            DarcyWeisbach().diameter_check(
                hydraulic_parameters
            )  # Check: head loss does not exceed the max allowed head loss - increase D until reached
    ...
    def diameter_check(self, hydraulic_parameters):  #
        ...
        if max_headloss_allowed is None:
            max_headloss_allowed = (
                10  # If a maximum head loss allowed is not used, the default is 10%
            )
        ...
        if hydraulic_parameters.penstock_length is None:
            hydraulic_parameters.penstock_design_headloss = hl = (
                hydraulic_parameters.head * max_headloss_allowed
            )  # update
            ...
            if hl > max_headloss_allowed * h:  # head loss larger than maximum allowed
                while (
                    hl > max_headloss_allowed * h
                ):  # iterate until head loss is less than the max allowed
                    hydraulic_parameters.penstock_design_diameter = (
                        hydraulic_parameters.penstock_design_diameter + 0.1
                    )  # increase by 0.1
                    DarcyWeisbach().penstock_headloss_calculator(
                        hydraulic_parameters
                    )  # Recursion to solve until head loss = desired head loss
                    hl = (
                        hydraulic_parameters.penstock_design_headloss
                    )  # update after each step

        hydraulic_parameters.penstock_diameter = (
            hydraulic_parameters.penstock_design_diameter
        )  # update
    ...
    def penstock_headloss_calculator_ts(self, hydraulic_parameters):
        ...
        if hydraulic_parameters.penstock_length is None:  # penstock lenght not provided
            raise ValueError(
                "The penstock lenght is required for head loss calculations"
            )
            ...
            hydraulic_parameters.head_loss = (
                ff * L * V**2 / (D * 2 * g)
            )  # D-W head loss for flow != design_flow - update
...
class HazenWilliamns(HeadLoss):  # Head loss calculator using Hazen-Williams
    ...
    def penstock_headloss_calculator(self, hydraulic_parameters):
        ...
        if (
            hydraulic_parameters.penstock_frictionfactor is None
        ):  # if a value of C is not given
            C = HW_RoughnessSelector().roughness_selector(
                material=hydraulic_parameters.penstock_material
            )  # HW roughness factor (C)
        ...
        if (
            hydraulic_parameters.penstock_diameter is not None
        ):  # Penstock diameter is known
            D = hydraulic_parameters.penstock_diameter  # Design diameter
            head_loss = 10.67 * L * Q**1.852 / (C**1.852 * D**4.87)  # HW head loss

        else:
            if (
                hydraulic_parameters.max_headloss_allowed is not None
            ):  # user-entered max head loss allowed
                head_loss = (
                    hydraulic_parameters.max_headloss_allowed
                    * hydraulic_parameters.head
                    / 100
                )
            else:
                head_loss = (
                    0.1 * hydraulic_parameters.head
                )  # default to 10% of head loss

            D = (10.67 * L * Q**1.852 / (C**1.852 * head_loss)) ** (
                1 / 4.87
            )  # Calculated diameter
            hydraulic_parameters.penstock_design_headloss = head_loss  # update
...
if __name__ == "__main__":
    ...
    # Head loss
    hyd_param = HydraulicDesignParameters(
        flow=1000,
        design_flow=1000,
        channel_average_velocity=None,
        head=100,
        penstock_length=115,
        penstock_diameter=10,
        head_loss=None,
        max_headloss_allowed=None,
        penstock_headloss_method=None,
