...
class EconomicParameters:
    def __init__(
        ...
        self.electricity_sell_price = (
            electricity_sell_price  # selling price of electricity, $/kWh
        )
        self.capacity_factor = capacity_factor  # capcity factor, %
        self.n_operation_days = (
            n_operation_days  # number of days the plant operates in a year
        )
...
def pd_checker(flow, flow_column):
    ...
    pandas_dataframe = (
        False  # flag to track if a pandas dataframe is used outside of this function
    )
    ...
    if isinstance(flow, pd.DataFrame):  # check if a pandas dataframe is used
        ...
        if flow_column is None:
            raise ValueError(
                "When using a pandas dataframe, users must indicate what column"
                " has the flow values using 'flow_column = \"Name\"'"
            )
        else:
            flow_data = flow[
                flow_column
            ].to_numpy()  # Extract flow from a pandas dataframe column
...
class Units:
    ...
    def us_to_si_conversion(self):
        ...
        if self.minimum_turbineflow is not None:
            self.minimum_turbineflow = (
                self.minimum_turbineflow * cfs_to_cms
            )  # cfs to m3/s
        ...
        if self.channel_average_velocity is not None:
            self.channel_average_velocity = (
                self.channel_average_velocity * ft_to_m
            )  # ft to m
    ...
    def si_to_us_conversion(self):
        ...
        if self.dataframe_output is not None:
            self.dataframe_output["turbine_flow_cfs"] = (
                self.dataframe_output["turbine_flow_cfs"] / cfs_to_cms
            )  # m3/s tp cfs
        ...
        if self.penstock_design_headloss is not None:
            self.penstock_design_headloss = (
                self.penstock_design_headloss / ft_to_m
            )  # m to ft
...
class Basic(Hydropower):
    ...
    def hydropower_calculation(self, hp_params):
        ...
        if len([i for i in [flow, head, rated_power] if i is None]) > 1:
            raise ValueError(
                "Missing inputs. Users must provide flow and head to compute power, flow and power"
        ...
        hp_params.penstock_design_headloss = (
            None  # update - for using the same Unit function
        )
...
class Diversion(Hydropower):
    ...
    def hydropower_calculation(self, hp_params):
        ...
        if turbine_type not in tt_list:
            raise ValueError(
                "The following turbine types are currently supported in HydroGenerate",
                tt_list,
            )
        ...
        if (
            hp_params.penstock_headloss_calculation
        ):  # If head loss in the penstock are calculated

            if hp_params.penstock_length is None:
                raise ValueError(
                    "Penstock length is required for head loss computations if"
            ...
            if hl_method not in hl_list:
                raise ValueError(
                    "The following head loss methods are currently supported in HydroGenerate",
                    hl_list,
                )

            if hl_method == "DARCY-WEISBACH":  # Darcy-Weisbach
                DarcyWeisbach().penstock_headloss_calculator(
                    hp_params
                )  # head loss at design parameters
                DarcyWeisbach().penstock_headloss_calculator_ts(
                    hp_params
                )  # head loss for a range of flow values

            elif hl_method == "HAZEN-WILLIAMS":  # Hazen-Williams
                HazenWilliamns().penstock_headloss_calculator(
                    hp_params
                )  # head loss at design parameters
                HazenWilliamns().penstock_headloss_calculator_ts(
                    hp_params
                )  # head loss for a range of flow values

        else:
            hp_params.head_loss = 0
            hp_params.penstock_headloss_method = (
                None  # penstock head losses are not calculated
            )
        ...
        if hp_params.generator_efficiency is None:
            ...
            hp_params.generator_efficiency = (
                hp_params.generator_efficiency / 100
            )  # percent to proportion

        # remove negatives and > 1 from turbine effiency
        hp_params.turbine_efficiency = np.where(
            hp_params.turbine_efficiency < 0, 0, hp_params.turbine_efficiency
        )  # remove negatives
        hp_params.turbine_efficiency = np.where(
            hp_params.turbine_efficiency > 1, 1, hp_params.turbine_efficiency
        )  # remove > 1

        n = (
            hp_params.turbine_efficiency * hp_params.generator_efficiency
        )  # overal system efficiency
        n_max = (
            np.nanmax(hp_params.turbine_efficiency) * hp_params.generator_efficiency
        )  # maximum system efficiency

        if (
            hp_params.penstock_design_headloss
        ):  # if design head loss was calculated for penstock only
            hd = (
                hp_params.head - hp_params.penstock_design_headloss
            )  # net hydraulic head at design flows
        ...
        P = (
            n_max * g * rho * Qd * hd / 1000
        )  # HP potential in kilowatts for design flow
...
class Hydrokinetic(Hydropower):
    ...
    def hydropower_calculation(self, hp_params):
        ...
        if hp_params.system_efficiency:  # if the user provides an efficiency value
            ...
            n = hp_params.system_efficiency = (
                59  # Efficiency [percentage] - Betz Limit - Maximum
            )
        ...
        # Area of blades
        if (
            hp_params.hk_swept_area is None
        ):  # If the user does not input swept area of blades
            Hydrokinetic_Turbine().turbine_calculator(
                hp_params
            )  # calculate swept area of blades
...
class ONRL_BaselineCostModeling_V2(Cost):
    ...
    def cost_calculation(self, hp_params):
        ...
        if res_category not in rc_list:
            raise ValueError(
                "The resource category must be in the following list", rc_list
            )
        ...
        if res_category == "PSH_ EXISTINGINFRAESTRUCTURE":
            icc = (
                3008246 * 𝑃 * exp(-0.000460 * P)
            )  # Pumped Storage Hydropower Projects - Existing Infrastructure

        if res_category == "PSH_GREENFIELD":
            icc = (
                4882655 * 𝑃 * exp(-0.000776 * P)
            )  #  Pumped Storage Hydropower Projects - Greenfield Sites
        ...
        annual_om = (
            225417 * 𝑃**0.54 / 1000000
        )  # Annual Operation Maintennance, million $
...
class ConstantEletrictyPrice(Revenue):
    ...
    def revenue_calculation(self, hp_params):
        ...
        mean_power = np.mean(
            hp_params.power
        )  # mean of the power provided for a time series of flow
        ...
        if hp_params.capacity_factor:
            annual_energy = (
                hp_params.capacity_factor * mean_power * 365
            )  # annual energy generated, killowatt day
            hp_params.n_operation_days = hp_params.capacity_factor * 365  # update

        else:
            if hp_params.n_operation_days > 365:
                raise ValueError(
                    "The number of days in a year a plant operates cannot exceed 365"
                )

            annual_energy = (
                hp_params.n_operation_days * mean_power
            )  # annual energy generatedkillowatt day
            hp_params.capacity_factor = hp_params.n_operation_days * 100 / 365  # update

        if hp_params.electricity_sell_price is None:
            hp_params.electricity_sell_price = (
                wholesale_elecprice_2023  # electricity sell price, defined above
            )

        hp_params.annual_energy_generated = (
            annual_energy * 24
        )  # units from Kw day to KWh
        hp_params.annual_revenue = (
            hp_params.annual_energy_generated
            * hp_params.electricity_sell_price
            / 1000000
        )  # annual revenue, M$
...
class ConstantEletrictyPrice_pd(Revenue):
    ...
    def revenue_calculation(self, hp_params, flow):
        ...
        output["turbine_flow_cfs"] = (
            hp_params.turbine_flow
        )  # Flow passing by the turbine, cfs
        ...
        hours = flow.index.to_series().diff().values / pd.Timedelta(
            "1 hour"
        ...
        flow_md = output.groupby([output.index.year]).agg(
            annual_turbinedvolume_ft3=("turbine_flow_cfs", "sum"),
        ...
        flow_md["revenue_M$"] = (
            flow_md["total_annual_energy_KWh"]
            * hp_params.electricity_sell_price
            / 1000000
        )
        flow_md["capacity_factor"] = flow_md["total_annual_energy_KWh"] / (
            hp_params.rated_power * 8760
        )  # energy generater / max energy. 1 year = 8760 hours
...
def calculate_hp_potential(
    ...
    flow_data, pandas_dataframe = pd_checker(
        flow, flow_column
    )  # check if a dataframe is used and extract flow values

    # initialize all instances: HydraulicDesign, Turbine, and Economic parameters.
    hyd_pm = HydraulicDesignParameters(
        flow=flow_data,
        design_flow=design_flow,
        head=head,  # net_head= net_head,
    ...
    turb_pm = TurbineParameters(
        ...
        pelton_n_jets=pelton_n_jets,
        hk_blade_diameter=hk_blade_diameter,
        hk_blade_heigth=hk_blade_heigth,
        hk_blade_type=hk_blade_type,
        hk_swept_area=hk_swept_area,
    )  # Initialize

    cost_pm = EconomicParameters(
        resource_category=resource_category,
        electricity_sell_price=electricity_sell_price,
        capacity_factor=capacity_factor,
        n_operation_days=n_operation_days,
    )

    flow_preproc_pm = FlowPreProcessingParameters(
        minimum_turbineflow=minimum_turbineflow,
        minimum_turbineflow_percent=minimum_turbineflow_percent,
        annual_maintenance_flag=annual_maintenance_flag,
        major_maintenance_flag=major_maintenance_flag,
    )

    all_params = merge_instances(
        hyd_pm, turb_pm, cost_pm, flow_preproc_pm
    )  # merge parameters into a single instance
    ...
    if pandas_dataframe:
        all_params.datetime_index = (
            flow.index
        )  # Obtain index from flow data for annual calculation

    # check if units are in list
    if units not in ["SI", "US"]:
        raise ValueError(
            'Units can only be "US" or "SI", see HydroGenerate for additional information'
        )

    # units conversion - US to Si
    if units == "US":
        Units.us_to_si_conversion(
            all_params
        )  # convert imputs from US units to SI units
    ...
    if annual_caclulation:
        if pandas_dataframe:  # If a pandas dataframe is used
            ConstantEletrictyPrice_pd().revenue_calculation(
                all_params, flow=flow
            )  # calculate revenue when a time-indexed pd.dataframe is given.
